...
class StatusPage:
    ...
    def populate_status_page(self):
        ...
        self.operator.populate_ec2_instance()
        # AWS status
        ...
        if self.operator.check_aws_credentials(show_result=False):
            ...
            aws_conn_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'              Account: ' + self.operator.aws_account_number + "\n"
            aws_conn_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'               Key ID: ' + self.operator.key_id + "\n"
            aws_conn_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'            Secret ID: ' + self.operator.secret_id
        ...
        if self.operator.get_instance() and self.operator.get_web_url():
            ...
            pipeline_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'      e-commerce site: ' + self.operator.k8_website + "\n"
            pipeline_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'     EKS Cluster Name: ' + 'madzumo-ops-cluster' + "\n"
            pipeline_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'   EKS Cluster status: ' + self.operator.get_cluster_status() + "\n"
            pipeline_info += Back.BLACK + Fore.LIGHTWHITE_EX + f'        Operator Node: ' + self.operator.ec2_instance_public_ip + "\n"
